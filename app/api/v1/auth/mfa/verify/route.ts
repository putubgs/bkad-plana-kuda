...
import { clearPendingMfaChallenge, getPendingMfaChallenge } from "@/lib/auth/mfa-pending";
...
export async function POST(request: NextRequest) {
  ...
  const challenge = await getPendingMfaChallenge();
  if (!challenge) {
    return NextResponse.json<ApiResult>(
      { error: "Sesi verifikasi telah kedaluwarsa. Silakan login kembali." },
      { status: 400 }
    );
  }
